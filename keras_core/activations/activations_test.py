...
class ActivationsTest(testing.TestCase):
    ...
    def test_softsign(self):
        ...
        x = np.random.random((2, 5))
        result = activations.softsign([x])[0]
        expected = softsign(x)
        self.assertAllClose(result, expected, rtol=1e-05)
    ...
    def test_sigmoid(self):
        ...
        x = np.random.random((2, 5))
        result = activations.sigmoid([x])[0]
        expected = sigmoid(x)
        self.assertAllClose(result, expected, rtol=1e-05)
    ...
    def test_gelu(self):
        ...
        x = np.random.random((2, 5))
        result = activations.gelu([x])[0]
        expected = gelu(x)
        self.assertAllClose(result, expected, rtol=1e-05)

        x = np.random.random((2, 5))
        result = activations.gelu([x], approximate=True)[0]
        expected = gelu(x, True)
        self.assertAllClose(result, expected, rtol=1e-05)

    def test_elu(self):
        x = np.random.random((2, 5))
        result = activations.elu([x])[0]
        self.assertAllClose(result, x, rtol=1e-05)
        negative_values = np.array([[-1, -2]], dtype=backend.floatx())
        result = activations.elu([negative_values])[0]
        true_result = np.exp(negative_values) - 1
        self.assertAllClose(result, true_result)

    def test_tanh(self):
        x = np.random.random((2, 5))
        result = activations.tanh([x])[0]
        expected = np.tanh(x)
        self.assertAllClose(result, expected, rtol=1e-05)

    def test_exponential(self):
        x = np.random.random((2, 5))
        result = activations.exponential([x])[0]
        expected = np.exp(x)
        self.assertAllClose(result, expected, rtol=1e-05)

    def test_mish(self):
        x = np.random.random((2, 5))
        result = activations.mish([x])[0]
