...
def load_balanced_loss(router_probs, expert_mask):
    ...
    loss = ops.mean(density_proxy * density) * ops.cast(
        (num_experts**2), "float32"
    )
    return loss
...
class Router(layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            router_logits += ops.random.uniform(
        ...
        expert_mask *= ops.cast(
            ops.less(
                ops.cast(position_in_expert, "int32"), self.expert_capacity
            ),
            "float32",
        )
...
class Switch(layers.Layer):
    ...
    def unstack(self, tensor, axis=0):
        shape = tensor.shape
        n = shape[axis]
        output_shape = [shape[:axis]] + [n] + [shape[axis + 1:]]
        unpacked = ops.split(tensor, n, axis=axis)
        return unpacked
    ...
    def call(self, inputs):
        ...
        # Dispatch to experts
        expert_input_list = self.unstack(expert_inputs, axis=0)
        expert_output_list = [
            self.experts[idx](expert_input)
            for idx, expert_input in enumerate(expert_input_list)
        ]
        # expert_outputs shape: [expert_capacity, num_experts, embed_dim]
        expert_outputs = ops.squeeze(ops.stack(expert_output_list, axis=2))
