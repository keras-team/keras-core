...
An implementation of EfficientNet B0 to B7 has been shipped wit keras since TF2.3. To
...
with strategy.scope():
    ...
    outputs = efficientnet.EfficientNetB0(include_top=True, weights=None, classes=NUM_CLASSES)(x)
...
def build_model(num_classes):
    inputs = layers.Input(shape=(IMG_SIZE, IMG_SIZE, 3))
    x = img_augmentation(inputs)
    model = efficientnet.EfficientNetB0(include_top=False, input_tensor=x, weights="imagenet")
